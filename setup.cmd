...
if "%SELECTED_LANG%"=="ES" (
    ...
    set "MSG_STATUS_CLI=CLI: Nightly (v0.33.0-nightly.20260302)"
...
echo   modelConfigs: { >> "%TEMP_SCRIPT%"
echo     "thinking-pro": { >> "%TEMP_SCRIPT%"
echo       model: "gemini-2.0-flash-thinking-exp-01-21", >> "%TEMP_SCRIPT%"
echo       generateContentConfig: { >> "%TEMP_SCRIPT%"
echo         thinkingConfig: { includeThoughts: true, thinkingBudget: 16000 } >> "%TEMP_SCRIPT%"
echo       } >> "%TEMP_SCRIPT%"
echo     }, >> "%TEMP_SCRIPT%"
echo     "elite-coding": { >> "%TEMP_SCRIPT%"
echo       model: "gemini-3-flash-preview", >> "%TEMP_SCRIPT%"
echo       generateContentConfig: { temperature: 0.2, maxOutputTokens: 8192 } >> "%TEMP_SCRIPT%"
echo     } >> "%TEMP_SCRIPT%"
echo   }, >> "%TEMP_SCRIPT%"
echo   general: { >> "%TEMP_SCRIPT%"
echo     defaultApprovalMode: "plan", >> "%TEMP_SCRIPT%"
echo     checkpointing: { enabled: true }, >> "%TEMP_SCRIPT%"
echo     sessionRetention: { maxAge: "7d", maxCount: 100 }, >> "%TEMP_SCRIPT%"
echo     previewFeatures: true, >> "%TEMP_SCRIPT%"
echo     plan: { modelRouting: true } >> "%TEMP_SCRIPT%"
echo   }, >> "%TEMP_SCRIPT%"
echo   experimental: { >> "%TEMP_SCRIPT%"
echo     enableAgents: true, >> "%TEMP_SCRIPT%"
echo     jitContext: true, >> "%TEMP_SCRIPT%"
echo     toolOutputMasking: true, >> "%TEMP_SCRIPT%"
echo     skillCreator: { >> "%TEMP_SCRIPT%"
echo        enabled: true, >> "%TEMP_SCRIPT%"
echo     }, >> "%TEMP_SCRIPT%"
echo     hooks: true, >> "%TEMP_SCRIPT%"
echo     eventDrivenScheduler: true >> "%TEMP_SCRIPT%"
echo   }, >> "%TEMP_SCRIPT%"
echo   tools: { >> "%TEMP_SCRIPT%"
echo     truncateToolOutputThreshold: 30000, >> "%TEMP_SCRIPT%"
echo     disableLLMCorrection: true, >> "%TEMP_SCRIPT%"
echo     shell: { showColor: true } >> "%TEMP_SCRIPT%"
echo   }, >> "%TEMP_SCRIPT%"
echo   ui: { >> "%TEMP_SCRIPT%"
echo     inlineThinkingMode: "full", >> "%TEMP_SCRIPT%"
echo     showStatusInTitle: true, >> "%TEMP_SCRIPT%"
echo     dynamicWindowTitle: true, >> "%TEMP_SCRIPT%"
echo     footer: { hideContextPercentage: false, showSandboxStatus: true }, >> "%TEMP_SCRIPT%"
echo     showMemoryUsage: true, >> "%TEMP_SCRIPT%"
echo     showModelInfoInChat: true >> "%TEMP_SCRIPT%"
echo   }, >> "%TEMP_SCRIPT%"
echo   agents: { >> "%TEMP_SCRIPT%"
echo     generalist: { enabled: true, modelAlias: "elite-coding" }, >> "%TEMP_SCRIPT%"
echo     codebaseInvestigator: { enabled: true, modelAlias: "thinking-pro" } >> "%TEMP_SCRIPT%"
echo   }, >> "%TEMP_SCRIPT%"
echo   context: { >> "%TEMP_SCRIPT%"
echo     fileFiltering: { respectGitIgnore: true }, >> "%TEMP_SCRIPT%"
echo     loadMemoryFromIncludeDirectories: true, >> "%TEMP_SCRIPT%"
echo     discoveryMaxDirs: 500 >> "%TEMP_SCRIPT%"
echo   } >> "%TEMP_SCRIPT%"
echo }; >> "%TEMP_SCRIPT%"
...
echo ----------------------------------------------------
echo STATUS: Nightly (v0.33.0-Windows) ^| Skills: %SKILLS_COUNT%
echo ----------------------------------------------------
pause